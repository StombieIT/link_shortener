...
class LinkControllerIntegrationTest {
    ...
    void deleteLink_ShouldReturn200() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.delete("/" + slug)
                        .header("X-User-Id", testUserId))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.result").value("Link '" + slug + "' has been deleted"));
    }
    ...
    void editLink_ShouldUpdateLimit() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.put("/" + slug)
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("X-User-Id", testUserId)
                        .content(jsonEdit))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.result").value("Link '" + slug + "' has been edited successfully"));
    }

    @Test
    void getNonExistentLink_ShouldReturn404() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/nonexistentSlug"))
